var Menu = function()
{
	...
	// Construct.prototype = {
		...
		Construct.prototype.init = function(params)
		{
			...
			if (!params.menuID || !params.buttonCheckerID || !params.headerHeight || !params.items)
			{
	    ...
	    params = null;
			...
			// this.menu.classList.add('hor-menu');
		...
		Construct.prototype.activateSectionLink = function()
		{
			...
			{
				document.querySelectorAll('#' + this.menuID + ' a')[i].classList.remove('active');
			}
			// console.log(this.items[activeSectionIndex]);
			document.querySelectorAll('#' + this.menuID + ' a')[activeSectionIndex].classList.add('active');
