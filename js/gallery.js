var Gallery = function()
{
  ...
  Construct.prototype = Object.create(ReasanikBase());
	...
	Construct.prototype.changeSlide = function(direction)
	{
		...
		imgContainer.innerHTML = '<picture><source srcset="' + this.rootFolder + '/d/' + tempImageName + '" media="(min-width: 768px)"><source srcset="' + this.rootFolder + '/t/' + tempImageName + '" media="(min-width: 320px)"><img srcset="' + this.rootFolder + '/m/' + tempImageName + '" alt="' + this.items[index].title + '"></picture>';
	...
	Construct.prototype.getItems = function(catIndex)
	{
		...
		for (i = 0, len = this.items.length; i < len; i++)
		{
			...
			// tempInnerHTML += '<div class="' + this.childClass + '"><div class="photo-block-frame"><div class="photo-block"><img src="' + this.rootFolder + (i + 1) + '.jpg" alt="' + this.items[i].title + '"><div class="foreground"></div><h3>' + this.items[i].title + '</h3></div></div>';

			tempImageName = (i + 1) + '.jpg';

			tempInnerHTML += '<div class="' + this.childClass + '"><div class="photo-block-frame"><div class="photo-block"><picture><source srcset="' + this.rootFolder + '/d/' + tempImageName + '" media="(min-width: 1170px)"><source srcset="' + this.rootFolder + '/t/' + tempImageName + '" media="(min-width: 768px)"><img srcset="' + this.rootFolder + '/m/' + tempImageName + '" alt="' + this.items[i].title + '"></picture><div class="foreground"></div><h3>' + this.items[i].title + '</h3></div></div>';

			if (this.showCategory)
			{
				tempInnerHTML += '<div class="rhombus-wrap button category blog"><div class="rhombus"></div></div>';
			}

			tempInnerHTML += '<a href="#" data-index="' + i + '" class="more button rhombus-wrap"><div class="rhombus"></div></a></div>';
	...
	Construct.prototype.createMenu = function()
	{
		...
		document.querySelector('#' + this.name + ' .container').insertBefore(container, document.getElementById(this.name + '-blocks'));
	};
	...
	Construct.prototype.randomShowing = function()
	{
		...
		elem = document.querySelector('#' + this.name + '-blocks .' + this.childClass + ':nth-child(' + (index + 1) + ')');
