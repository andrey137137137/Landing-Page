var Slider = function()
{
	...
	function Construct(params)
	{
		...
		var timingMethods = {
			...
			ease: function()
			{
				if (changingStyles[timingStyle].value > 30 && timingDirection < 0 ||
						changingStyles[timingStyle].value < -30 && timingDirection > 0)
				{
					changingStyles[timingStyle].step = 5;
				}
				else if (changingStyles[timingStyle].value > 20 && timingDirection < 0 ||
						changingStyles[timingStyle].value < -20 && timingDirection > 0)
				{
					changingStyles[timingStyle].step = 2;
				}
				else if (changingStyles[timingStyle].value > 10 && timingDirection < 0 ||
			...
			quad: function()
			{
				changingStyles[timingStyle].step = Math.pow(changingStyles[timingStyle].step, 2) + changingStyles[timingStyle].step;
				changeStyleValue();
			}
		};
		...
		document.getElementById(getSlideID(curIndex)).classList.add('active');
		...
		function setThumb(elem)
		{
			if (!animationID)
			{
				...
				prevIndex = document.querySelector('#' + navButtons.thumbs + ' .active').dataset.number;
				...
				// thumbs[elem.dataset.number].classList.add('active');
