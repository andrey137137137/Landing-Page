  ...
  Construct.prototype = Object.create(ReasanikBase());
	...
	// Construct.prototype = {
	    ...
	    params = null;
			...
			var elems = document.querySelectorAll('#' + this.formID + ' .require');
		...
		Construct.prototype.validate = function(elem)
		{
			...
			switch (tag)
			{
				case 'select': type = tag;
											 break;
				default: type = elem.getAttribute('type') || 'text';
			}

			switch (type)
			{
				case 'select': validateSelect(elem);
											 break;
				case 'text': 	if (elem.name == this.formID + '_website')
											{
											...
											break;
				case 'email': this.validateEmail(elem);
										 break;
				case 'password': validatePassword(elem);
										 break;
			}
